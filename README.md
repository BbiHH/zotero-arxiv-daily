...
llm:
  ...
  filter:
    ...
    max_batches: 20 # Maximum batches; 5 x 20 covers the embedding Top 100.
...
`generation_kwargs` only overrides request parameters needed for scoring.
...
## 📖 How it works
*Zotero-arXiv-Daily* firstly retrieves all the papers in your Zotero library and all the papers released in the previous day, via corresponding API. Then it calculates the embedding of each paper's abstract via an embedding model. By default, a candidate's score is its mean similarity to the filtered Zotero corpus. With collection priority configured, similarities are averaged within each collection first and then combined by normalized collection weights. The optional LLM filter turns the embedding Top 100 into a smaller, configurable email set such as Top 30. The TLDR of each selected paper is generated by LLM, given the text extracted by pymupdf4llm.
